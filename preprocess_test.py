...
def preprocess(src_dir: Path, data_dir: Path, preprocessed_dir: Path) -> dict[Hashable, Any]:
    ...
    metadata_path = os.path.join(data_dir, 'metadata_TdPVeJC.csv')
    metadata = pd.read_csv(metadata_path)
    # Train set
    target_tr_path = os.path.join(data_dir, 'train_1128update.csv')
    ...
    monthly_NSF_tr_path = os.path.join(data_dir, 'train_monthly_naturalized_flow_1128update.csv')
    monthly_NSF_test_path = os.path.join(data_dir, 'test_monthly_naturalized_flow_1128update.csv')
    ...
    Data_folder = os.path.join(data_dir, 'Updated')
    ...
    for date in forecast_date:
        ...
        pred_model_10 = load("trained_models/"+date+"_"+str(0.1)+"_model.dat")
        pred_model_50 = load("trained_models/" + date + "_" + str(0.5) + "_model.dat")
        pred_model_90 = load("trained_models/" + date + "_" + str(0.9) + "_model.dat")
        ...
        for site in site_id_short:
            ...
            SWE_df = read_predictors(SWE_folder_path + '/%s.csv' % site, site, predictor_is_SWE=True)
            PPT_df = read_predictors(PPT_folder_path + '/%s.csv' % site, site)
            Tmax_df = read_predictors(Tmax_folder_path + '/%s_Tmax.csv' % site, site)
            Tmin_df = read_predictors(Tmin_folder_path + '/%s_Tmin.csv' % site, site)
            NSF_train_df = read_predictors(monthly_NSF_tr_path, site, predictor_is_NSF=True)
            NSF_test_df = read_predictors(monthly_NSF_test_path, site, predictor_is_NSF=True)
            ...
            NSF_df_tr.insert(0, 'site_id', get_site_full_id(site))
            ...
            NSF_df_test.insert(0, 'site_id', get_site_full_id(site))
            ...
            drainage_area = metadata[metadata['site_id'] == get_site_full_id(site)]['drainage_area']
    ...
    # Save X and y
    #input_df.to_csv('preprocessed_dir/features.csv', index=False)
    #target_NSF_df_tr.to_csv('preprocessed_dir/target.csv', index=False)

    '''
    with open('all_gradient_boosting_params.json', 'r') as json_file:
